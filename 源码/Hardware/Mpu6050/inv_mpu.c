...
//下面8个函数对接dmp库
#define HAL_Delay    HAL_Delay       //delay实现
#define get_ms(p)       do{*p=HAL_GetTick();}while(0)        //自己写一个
...
//返回值:0,正常
//    其他,失败
u8 MPU6050_DMP_Get_Data(float *pitch,float *roll,float *yaw,short *gyro,short * accel)//（5MS  200HZ调用一下，给 DEFAULT_MPU_HZ 频率保持一致，或者int中断引脚读取）
